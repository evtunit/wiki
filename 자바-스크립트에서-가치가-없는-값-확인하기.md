...
대조적으로 undefined과는 달리, null은 리터럴이다. 일부 속성의 프로퍼티가 아닙니다. 그것은 신원의 미상을 나타냅니다.

이러한 특성을 바탕으로이 두 말을 안전 undefined하고 null값이 없음을 나타냅니다. 따라서 값의 부재 여부를 확인하기 위해 작성한 코드는 모두 undefined와 null를 고려해야합니다.
...
이 솔루션은 잠재적으로 오류를 던지는 코드가 If 조건과 일치하기 때문에 ReferenceError도 던지지 않는 경우에만 작동합니다. 나는 아무 이유없이이 일을 의도적으로 생각할 수 없다. 이 상황은 선언 된 변수 이름의 철자 오류로 인해 발생할 수 있습니다. 이런 이유로 try catch 블록에 코드를 가능한 한 짧게 유지해야합니다. if 조건은 ReferenceError 메시지의 문자열을 검사하여 특정 변수 err instanceof ReferenceError && err.message.split ( '') [0] === 'value'를 검사 할 수도 있지만, 권장하지 않기 때문에 추천하지 않습니다. 코드의 디버깅과 수정이 필요한 변수 이름의 철자가 잘못되었습니다.
...
- 엄격한 평등은 둘 다 피연산자 유형을 검사하기 때문에 느슨한 평등보다 느립니다.
- 엄격한 동등성은 피연산자 유형을 강제 변환하지 않고 즉시 false를 반환 할 수 있기 때문에 피연산자 유형이 다른 경우 느슨한 동등성보다 빠릅니다.
- 느슨한 평등은 예기치 않은 결과를 낳기 때문에 가능하면 피해야합니다.

두 번째 글 머리표는 값이 null과 동일한 유형이 아니기 때문에 두 번째 조건에 완전 항등 사용에 대한 강력한 인수를 사용하지만 첫 번째 조건에서는 typeof value와 'undefined'가 모두 string 유형으로 보장되므로 엄격한 평등 사용은 첫 번째 및 세 번째 글 머리 점에서만 지원됩니다. 이렇게하면 첫 번째 표현이 최상의 선택입니다.
